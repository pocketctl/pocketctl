...
type JSONLEntry struct {
	...
	UUID           string        `json:"uuid,omitempty"`       // stable record identity — the preferred turn anchor (never a content hash)
...
type JSONLMessage struct {
	...
	StopReason string          `json:"stop_reason,omitempty"` // assistant terminal evidence (end_turn, tool_use, …) — preserved, not guessed
...
// synthetic→receipt and compact-status logic as ClaudeAdapter. Turn identity
// is derived conservatively from record uuids: a user record starts (or
// addends) a turn, the result record closes it as completed, an interruption
// marker closes it as interrupted; incomplete records never guess completion.
func (p *JSONLStreamParser) Parse(line string) ([]protocol.DaemonEvent, error) {
	...
	case "user":
		...
		events, err := parseUserJSONL(entry, sid)
	...
	case "result":
		...
		if done, ok := p.turns.end(sid, protocol.TurnStateCompleted, "result_record", entry.IsSidechain); ok {
			events = append([]protocol.DaemonEvent{done}, events...)
		}
		return events, nil
...
// beginTurn anchors a new turn on the user record's own uuid when no turn is
// active. A missing uuid keeps the stream unassigned (no guessing — the
// parentUuid is a message-level relation, not this record's identity).
func (p *JSONLStreamParser) beginTurn(sid string, entry JSONLEntry, subagent bool) (protocol.DaemonEvent, bool) {
	return p.turns.begin(sid, entry.UUID, subagent)
}

func (p *JSONLStreamParser) stampTurns(events []protocol.DaemonEvent, subagent bool) {
	for i := range events {
		p.turns.stamp(&events[i], subagent)
	}
}

// isEntryPureInterruptMarker checks the user record's textual content for the
// interruption marker.
func isEntryPureInterruptMarker(entry JSONLEntry) bool {
	if entry.Message == nil {
		return isPureInterruptMarker(entry.Content)
	}
	var textStr string
	if json.Unmarshal(entry.Message.Content, &textStr) == nil && textStr != "" {
		return isPureInterruptMarker(textStr)
	}
	var blocks []JSONLContentBlock
	if json.Unmarshal(entry.Message.Content, &blocks) == nil {
		for _, b := range blocks {
			if b.Type == "text" && isPureInterruptMarker(b.Text) {
				return true
			}
		}
	}
