...
//
// 实现原理：起 `caffeinate -i -s -w <daemon-pid>` 子进程持续持有休眠抑制。
//   - `-i` 阻止系统空闲休眠。
//   - `-s` 在交流电供电时阻止系统休眠，包括显示器关闭后的系统休眠。
...
func newCaffeinateCommand(pid int) *exec.Cmd {
	return exec.Command("caffeinate", "-i", "-s", "-w", strconv.Itoa(pid))
}
...
func (c *caffeinateInhibitor) Acquire() error {
	...
	cmd := newCaffeinateCommand(os.Getpid())
