...
func (c *codexCoordinator) shutdown() error {
	...
	// Keep ownership even when the idle native process is stopped. Recovery
	// must rebuild detached backends so remote clients can resume these threads.
	keepState := c.projectCwd != "" || len(c.managedThreadSnapshot()) > 0
	var err error
	if keepState {
		err = c.persistOwnerLocked(0)
	}
