...
func TestCodexProjectionThreadAndTurnLifecycle(t *testing.T) {
	...
	if len(events) != 2 || events[0].Type != protocol.EventTypeTurnStatus || events[0].TurnStatus != protocol.TurnStateRunning ||
		events[0].SourceTurnID != "turn_1" || events[0].TurnID != logicalCodexTurnID("thr_1", "turn_1") ||
		events[1].Type != "session_status" || events[1].SessionID != "thr_1" || events[1].Status != protocol.StatusRunning {
	...
	if len(events) != 2 || events[0].Type != protocol.EventTypeTurnStatus || events[0].TurnStatus != protocol.TurnStateCompleted ||
		events[1].Type != "session_status" || events[1].SessionID != "thr_1" || events[1].Status != protocol.StatusIdle {
...
func TestCodexProjectionTurnCompletionKeepsThreadWritable(t *testing.T) {
	turnStatus := func(threadID, native, state string) protocol.DaemonEvent {
		return protocol.DaemonEvent{Type: protocol.EventTypeTurnStatus, SessionID: threadID, TurnStatus: state}
	}
	tests := []struct {
		native string
		want   []protocol.DaemonEvent
	}{
		{native: "completed", want: []protocol.DaemonEvent{
			turnStatus("thr_completed", "turn_completed", protocol.TurnStateCompleted),
			{Type: "session_status", SessionID: "thr_completed", Status: protocol.StatusIdle}}},
		{native: "interrupted", want: []protocol.DaemonEvent{
			turnStatus("thr_interrupted", "turn_interrupted", protocol.TurnStateInterrupted),
			{Type: "session_status", SessionID: "thr_interrupted", Status: protocol.StatusIdle}}},
		{native: "failed", want: []protocol.DaemonEvent{
			turnStatus("thr_failed", "turn_failed", protocol.TurnStateFailed),
		...
		{native: "inProgress", want: []protocol.DaemonEvent{
			turnStatus("thr_inProgress", "turn_inProgress", protocol.TurnStateRunning),
			{Type: "session_status", SessionID: "thr_inProgress", Status: protocol.StatusRunning}}},
	}
...
func TestCodexProjectionHistoricalAndLiveFailedTurnProvenance(t *testing.T) {
	...
	for generation, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(live) != 3 || live[0].Type != protocol.EventTypeTurnStatus || live[0].TurnStatus != protocol.TurnStateFailed ||
				live[1].Type != "error" || live[2].Status != protocol.StatusIdle {
				t.Fatalf("live failure=%+v, want turn_status then error then idle", live)
			}
