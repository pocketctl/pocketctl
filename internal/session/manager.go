...
type SessionManager struct {
	...
	// Turn lifecycle (stage 2): registry is independent of sm.sessions and
	// never affects quota/ownership. turnMode comes from
	// POCKETCTL_TURN_ENRICHMENT=off|observe|enforce (default observe).
	turns          *turn.Registry
	turnMetrics    *TurnMetrics
	turnMode       turnEnrichmentMode
...
func NewSessionManagerWithTrustedActionPolicy(outputCh chan protocol.DaemonEvent, trustedActionPolicy string) *SessionManager {
	return &SessionManager{
		...
		turns:                  turn.NewRegistry(nil, nil),
		turnMetrics:            NewTurnMetrics(),
		turnMode:               parseTurnEnrichmentMode(os.Getenv("POCKETCTL_TURN_ENRICHMENT")),
