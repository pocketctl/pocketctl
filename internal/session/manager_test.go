...
func TestRequiresResumeLoadsHistoricalOpenCodeBeforeQuotaDecision(t *testing.T) {
	repo := t.TempDir()
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch {
		case r.Method == http.MethodGet && r.URL.Path == "/api/health":
			_ = json.NewEncoder(w).Encode(map[string]bool{"healthy": true})
		case r.Method == http.MethodGet && r.URL.Path == "/api/session/ses_quota":
			_ = json.NewEncoder(w).Encode(map[string]any{"data": map[string]any{
				"id": "ses_quota", "directory": repo, "agent": "build",
				"model": map[string]string{"providerID": "opencode", "id": "x-preview-f-free"},
			}})
		case r.Method == http.MethodGet && r.URL.Path == "/session/status":
			_ = json.NewEncoder(w).Encode(map[string]any{"ses_quota": map[string]string{"type": "idle"}})
		case r.Method == http.MethodGet && (r.URL.Path == "/session/ses_quota/message" || r.URL.Path == "/session/ses_quota/todo"):
			_ = json.NewEncoder(w).Encode([]any{})
		default:
			http.NotFound(w, r)
		}
	...
	if sm.RequiresResume("ses_quota") {
		t.Fatal("historical OpenCode backend was classified as a process resume")
	}
	if agent, ok := sm.GetSessionAgent("ses_quota"); !ok || agent != adapter.AgentOpencode {
		t.Fatalf("agent=%q ok=%v, want lazily loaded opencode", agent, ok)
	}
}
