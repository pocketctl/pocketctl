	...
	for scanner.Scan() {
		...
		for _, evt := range events {
			...
			if evt.Type == "session_status" {
				// Turn terminal must precede the session status on the wire.
				sm.observeAgentStatusForTurn(ps.SessionID, evt.Status)
			}
			sm.outputCh <- evt
		}
	}
...
func (sm *SessionManager) finalizeProcessExit(ctx context.Context, exitErr error, ps *ProcessState) {
	...
	exitReason := protocol.ExitReasonNormalExit
	if exitErr != nil {
		if ctx.Err() == context.Canceled {
			status = protocol.StatusKilled
			exitReason = protocol.ExitReasonSignalKill
		} else {
			status = protocol.StatusError
			exitReason = protocol.ExitReasonProcessCrash
		}
	}
	// The turn terminalizes before the session status is published; a
	// normally-completed turn was already closed by the agent's own
	// session_status event, so this only fires for missing-evidence exits.
	sm.terminalizeTurnOnExit(ps.SessionID, exitReason)
	sm.mu.Lock()
	ps.Status = status
...
func (sm *SessionManager) RequiresResume(sessionID string) bool {
	...
	if !ok {
		sm.mu.RUnlock()
		// Relay may retain an OpenCode session longer than discovery's live
		// polling window. Load that exact session from the shared serve before
		// classifying the request as a process resume; otherwise the daemon both
		// requests the wrong quota grant and reserves a default Claude turn.
		if !sm.EnsureOpencodeSessionLoaded(sessionID) {
			return true
		}
		sm.mu.RLock()
		ps, ok = sm.sessions[sessionID]
...
func (sm *SessionManager) sendToIdleTerminal(ctx context.Context, ps *ProcessState, content string) error {
	...
	go func() {
		...
		if waitErr != nil {
			...
			// Successful one-shot resume = a completed turn (the stream events
			// may already have closed it; terminalization is idempotent).
			sm.observeAgentStatusForTurn(ps.SessionID, protocol.StatusIdle)
		}
		sm.mu.Lock()
		...
		// Trigger notification callback only after a successful resume whose
		// turn actually completed — never for interrupted/failed/abandoned.
		if sm.OnNotifyTerminal != nil && sm.turnAllowsCompletionSideEffects(ps.SessionID) {
