...
const (
	...
	// A failed blocking Prompt may still have committed its caller-supplied user
	// message before the provider failed. Give two sync polls a chance to bind
	// that exact source before the request turn is failed by inference.
	opencodePromptFailureGrace = 2 * opencodeSyncInterval
...
func (b *serverBackend) Send(ctx context.Context, sessionID, content string) error {
	...
	// Reserve an exact native source identity without publishing it as observed
	// evidence. The adapter projection confirms this expectation only after the
	// actual message appears; a competing client's newer source cannot win the
	// registry race. Message history is required to preserve OpenCode's
	// lexicographic ordering contract for externally supplied message ids.
	sourceMessageID := ""
	if sm := b.coord.sm; sm.turnEnabled() {
		key := turn.ActorKey{SessionID: sessionID}
		if active, ok := sm.turns.Active(key); ok && sm.agentForTurn(active) == adapter.AgentOpencode && active.SourceTurnID == "" {
			if active.ExpectedSourceTurnID != "" {
				// An expectation means another POST already owns this native
				// identity. This call has no stable request/content proof that it is
				// the same dispatch, so reusing the primary key could merge two user
				// inputs into one OpenCode message.
				return fmt.Errorf("opencode dispatch pending source confirmation")
			} else {
				if historyErr != nil {
					return fmt.Errorf("opencode source identity preflight: %w", historyErr)
				}
				reserved, err := adapter.ReserveOpencodeMessageID(ctx, messages)
				if err != nil {
					return err
				}
				if _, err := sm.turns.ExpectSource(key, active.TurnID, reserved); err != nil {
					return fmt.Errorf("opencode source identity reservation: %w", err)
				}
...
// response after validating its assistant parent against the reserved source.
// It returns true when the result belonged to the current canonical turn.
func (sm *SessionManager) finishOpenCodePromptDispatch(sessionID, sourceMessageID string, result adapter.OpencodeMessageWithParts, promptErr error) bool {
	if !sm.turnEnabled() || sourceMessageID == "" {
		return false
	}
	key := turn.ActorKey{SessionID: sessionID}
	active, ok := sm.turns.Active(key)
	if !ok || (active.ExpectedSourceTurnID != sourceMessageID && active.SourceTurnID != sourceMessageID) {
		return false
	}
	if promptErr != nil {
		sm.outputCh <- protocol.DaemonEvent{
			...
			TurnOrigin:     active.Origin,
			TurnConfidence: active.Confidence,
			Error:          "opencode prompt 失败: " + promptErr.Error(),
		}
		// Prompt's HTTP error is not proof that OpenCode rejected the supplied
		// message id: the native user row can be committed before the provider
		// fails. Closing here lets syncLoop rediscover that row as a second turn.
		// Keep the exact source reservation active for a bounded reconciliation
		// window; native terminal evidence wins, otherwise the fallback below
		// closes the original request identity.
		go sm.confirmOpenCodePromptFailure(sessionID, sourceMessageID)
		return true
	}

	// A successful blocking POST proves which native user message was created,
	// but it is not the outbound lifecycle ordering authority. The sync loop
	// must publish the final content before its terminal projection; closing the
	// canonical turn here races that poll and splits one real turn across the
	// request and source-native identities.
	_, err := sm.turns.BindSource(key, active.TurnID, sourceMessageID)
	return err == nil
}

func (sm *SessionManager) confirmOpenCodePromptFailure(sessionID, sourceMessageID string) {
	time.Sleep(opencodePromptFailureGrace)
	key := turn.ActorKey{SessionID: sessionID}
	rec, ok := sm.turns.Active(key)
	if !ok || rec.State != protocol.TurnStateRunning {
		return
	}
	if rec.ExpectedSourceTurnID != sourceMessageID && rec.SourceTurnID != sourceMessageID {
		return
	}
...
func (b *serverBackend) Interrupt(sessionID string) error {
	...
	if err := b.coord.srv().Abort(context.Background(), sessionID); err != nil {
		return err
	}
	// Abort accepted: the turn moves to interrupt_requested first; the poller's
	// authoritative status (or the bounded inference fallback) terminalizes it.
	sm := b.coord.sm
	if sm != nil && sm.turnEnabled() {
		key := turn.ActorKey{SessionID: sessionID}
		if rec, ok := sm.turns.Active(key); ok {
			if _, terr := sm.turns.RequestInterrupt(key, protocol.TurnReasonUserRequested); terr == nil {
				sm.emitTurnStatus(rec, protocol.TurnStateInterruptRequested, protocol.TurnReasonUserRequested)
				go sm.confirmAbortInterrupt(sessionID)
			}
		}
	}
	return nil
...
func (sm *SessionManager) createOpencodeSession(ctx context.Context, config protocol.SessionConfig) (string, error) {
	...
	if config.Prompt != "" {
		if err := sm.SendMessageWithInput(ctx, UserMessageInput{SessionID: sid, Content: config.Prompt}); err != nil {
			...
			// the legacy client-visible error there instead of degrading to logs.
			if !sm.turnEnabled() {
				sm.outputCh <- protocol.DaemonEvent{
					Type: "error", SessionID: sid, Error: "opencode 发送初始消息失败: " + err.Error(),
				}
			}
			slog.Default().Warn("opencode initial prompt dispatch failed", "session", sid, "error", err)
		}
	}
	return sid, nil
}
