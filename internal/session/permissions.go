...
func (sm *SessionManager) InterruptSession(sessionID string) error {
	...
	// The codex managed backend drives its own registry transition inside
	// Interrupt (typed no-active-turn error, native confirmation).
	if ps.Backend != nil {
	...
	if ps.Source == "daemon" && ps.PTY != nil {
		...
		// terminal JSONL record, so the turn moves to interrupt_requested
		// first and terminalizes via bounded inference before the session
		// returns to idle.
		...
		if sm.turnEnabled() {
			...
			if rec, ok := sm.turns.Active(key); ok {
				if _, terr := sm.turns.RequestInterrupt(key, protocol.TurnReasonUserRequested); terr == nil {
					...
					go sm.confirmPTYInterrupt(sessionID)
					return nil
				}
			}
		}
	...
	// Terminal session: cancel the --resume subprocess. The turn moves to
	// interrupt_requested first so the resume goroutine's finalize path maps
	// the cancellation to interrupted (user intent), never failed(signal_kill)
	// — review P1-4.
	if sm.turnEnabled() {
		key := turn.ActorKey{SessionID: sessionID}
		if rec, ok := sm.turns.Active(key); ok {
