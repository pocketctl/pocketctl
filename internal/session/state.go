...
func (sm *SessionManager) SetSessionExited(sessionID string, exitReason string) {
	// Active turns terminalize before the session exit status is published.
	sm.terminalizeTurnOnExit(sessionID, exitReason)
	sm.mu.Lock()
	ps, ok := sm.sessions[sessionID]
	if !ok {
		sm.mu.Unlock()
...
func (sm *SessionManager) SetSessionStatus(sessionID, status string) {
	// An agent-sourced return to idle/completed closes the active turn first
	// so the turn terminal precedes the session status on the wire.
	if status == protocol.StatusExited || status == protocol.StatusError || status == protocol.StatusKilled {
		exitReason := ""
		switch status {
...
func (sm *SessionManager) observeJSONLSessionStatus(sessionID, status string, allowObserver bool) bool {
	...
	if status == protocol.StatusExited || status == protocol.StatusCompleted ||
		...
		// The forwarded event closes the turn; recording happens after the
		// status fields so the observed order matches the wire order.
		sm.observeAgentStatusForTurn(sessionID, status)
	}
	return true
}
