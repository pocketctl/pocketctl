...
func TestCursorStore_TurnCommitSurvivesAckReloadAndHydration(t *testing.T) {
	cs := newCursorStore(t)
	recorded := recordBatchAt(t, cs, "zcode-turn-durable", 1_000_000, PendingRecord{
		WireSessionID:    "zcode-turn-durable",
		Position:         SourcePosition{Kind: PositionMessageInsert, MessageSequence: 1},
		ExpectedEventIDs: []string{"turn-running-event"},
		Commit: SyncCommit{
			LastEventID: "turn-running-event",
			Turn:        &TurnCommit{Anchor: "native-user-1", State: "running"},
		},
	})
	if len(recorded.Records) != 1 || recorded.Records[0].Commit.CommitOrder == 0 {
		t.Fatalf("recorded turn commit missing canonical order: %+v", recorded)
	}
	if _, err := cs.AcknowledgeEventIDs([]string{"turn-running-event"}); err != nil {
		t.Fatal(err)
	}

	// Reload from disk, not the existing in-memory store.
	reloaded := NewCursorStoreAt(cs.Path())
	file := mustSnapshot(t, reloaded)
	sess := file.Sessions["zcode-turn-durable"]
	if sess.Sync.TurnAnchor != "native-user-1" || sess.Sync.TurnEmitted["native-user-1"] != "running" {
		t.Fatalf("durable turn checkpoint lost after ACK/reload: %+v", sess.Sync)
	}
	z, err := NewZcodeSyncFromSessionCursor(testSourceID, "zcode-turn-durable", sess)
	if err != nil {
		t.Fatal(err)
	}
	if z.state.turnAnchor != "native-user-1" || z.state.turnEmitted["native-user-1"] != "running" {
		t.Fatalf("hydrated projection lost active turn: %+v", z.state)
	}
}

func TestCursorSnapshotDeepCopiesTurnProjection(t *testing.T) {
	cs := newCursorStore(t)
	recordBatchAt(t, cs, "zcode-turn-copy", 1_000_000, PendingRecord{
		WireSessionID: "zcode-turn-copy",
		Position:      SourcePosition{Kind: PositionMetadata},
		SkippedReason: "metadata-only",
		Commit: SyncCommit{
			Turn: &TurnCommit{Anchor: "native-user-copy", State: "running"},
		},
	})
	first := mustSnapshot(t, cs)
	first.Sessions["zcode-turn-copy"].Sync.TurnEmitted["native-user-copy"] = "corrupted-by-reader"
	second := mustSnapshot(t, cs)
	if got := second.Sessions["zcode-turn-copy"].Sync.TurnEmitted["native-user-copy"]; got != "running" {
		t.Fatalf("snapshot leaked mutable turn map into store: %q", got)
	}
}
