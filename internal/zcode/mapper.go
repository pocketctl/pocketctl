...
func (mp *Mapper) TurnStatus(wireSessionID, nativeAnchorMessageID, state, reason string) protocol.DaemonEvent {
	logical := turn.LogicalTurnID("zcode", wireSessionID, "", "source_message", nativeAnchorMessageID)
	ev := protocol.DaemonEvent{
		Type:              protocol.EventTypeTurnStatus,
		SessionID:         wireSessionID,
		TurnID:            logical,
		SourceTurnID:      nativeAnchorMessageID,
		TurnStatus:        state,
		TurnReason:        reason,
		...
		EventID:           turn.StatusEventID(logical, state),
	}
	return ev
}
