...
func TestObserver_PagePendingPersistsOnceBeforeFirstEmit(t *testing.T) {
	...
	if res.Emitted != 7 { // session_discovered + 5 user_text + session_status
		t.Fatalf("emitted = %d, want 7: %v", res.Emitted, rec.ids())
	}
...
func TestObserver_BackpressureStopsLaterPageEvents(t *testing.T) {
	...
	cs := NewCursorStoreAt(storage + "/cursor.json")
	rec := newEmitRecorder()
	rec.accept.Store(2) // accept discovered + first user_text, reject the rest
	o := testObserver(t, storage, cs, rec.fn())
	res := o.pollOnce(context.Background())
	if !res.Deferred {
	...
	ids := rec.ids()
	if len(ids) != 2 {
		t.Fatalf("emitted %d events after backpressure, want 2: %v", len(ids), ids)
	}
...
func TestObserver_AckedInsertDoesNotRescanFromZero(t *testing.T) {
	...
	if res.Emitted != 1 || res.NewPending != 1 {
		t.Fatalf("second poll emitted %d / newPending %d, want 1/1 (insert keyset resumed from high-water)", res.Emitted, res.NewPending)
	}
}
