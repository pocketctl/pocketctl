...
func TestZcodeSync_PreviewMessageUserAndAssistantError(t *testing.T) {
	z := NewZcodeSync(testSourceID, "zcode-wire1")
	// user message with text parts: turn_status(running) anchored on the user
	// message id, then the stamped user_text.
	b, _ := z.PreviewMessage("m1", "wm1", ZcodeMessageData{
		Role:  "user",
		Parts: []ZcodePartData{{Type: "text", Text: "hello user"}},
	})
	if len(b.Events) != 2 || b.Events[0].Type != protocol.EventTypeTurnStatus ||
		b.Events[0].TurnStatus != protocol.TurnStateRunning || b.Events[0].SourceTurnID != "m1" {
		t.Fatalf("user message: %+v", b.Events)
	}
	if b.Events[1].Type != "user_text" || b.Events[1].Text != "hello user" || b.Events[1].TurnID != b.Events[0].TurnID {
		t.Fatalf("user text not stamped: %+v", b.Events[1])
	}
	// assistant error: attributed error first, then the failed terminal for the
	// same turn. Consumers must never see the terminal before its cause.
	b, _ = z.PreviewMessage("m2", "wm2", ZcodeMessageData{Role: "assistant", Error: &ZcodeError{Message: "boom"}})
	if len(b.Events) != 2 || b.Events[0].Type != "error" || b.Events[0].Error != "boom" {
		t.Fatalf("assistant error turn: %+v", b.Events)
	}
	if b.Events[1].Type != protocol.EventTypeTurnStatus || b.Events[1].TurnStatus != protocol.TurnStateFailed {
		t.Fatalf("assistant error: %+v", b.Events[1])
	}
	if b.Events[0].TurnID == "" || b.Events[0].TurnID != b.Events[1].TurnID ||
		b.Events[0].SourceTurnID != "m1" || b.Events[1].SourceTurnID != "m1" {
		t.Fatalf("error and terminal lost shared turn identity: %+v", b.Events)
	}
	if b.Commit.LastEventID != b.Events[1].EventID {
		t.Fatalf("commit chain head = %q, want terminal %q", b.Commit.LastEventID, b.Events[1].EventID)
	}
}
