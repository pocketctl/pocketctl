...
final class SessionDetailViewModel {
    ...
    /// Outbound user messages waiting for relay routing feedback.
    ...
    var canSendMessage: Bool {
        let basePolicyAllowsSend = SessionInputPolicy.canSend(
            ...
            daemonOnline: wsService.isDaemonOnline(session.daemonId),
    ...
    func sendMessage(_ text: String) -> Bool {
        ...
        pendingUserMessages[msgId] = text
        wsService.send([
            "type": "user_message",
            "session_id": session.sessionId,
            "content": text,
    ...
    private func handleEvent(_ dict: [String: Any]) {
        ...
        if event.type == .userMessageAck {
            if let msgId = event.msgId {
                pendingUserMessages.removeValue(forKey: msgId)
            }
