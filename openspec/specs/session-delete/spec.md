...
Relay SHALL delete the session record and its events from the database, and insert a tombstone record into `deleted_sessions` table.

#### Scenario: Relay processes session_delete
- **WHEN** Relay receives `session_delete` message with a valid `session_id`
- **THEN** Relay deletes all events for that session from `events` table
- **AND** Relay deletes the session from `sessions` table
- **AND** Relay inserts the session_id into `deleted_sessions` table
- **AND** Relay sends `session_deleted` response to the originating client
