...
import { isReadOnlyObserverAgent } from './observerSession'
 ...
 *   - observer agents → null (no resume, never fall back to claude)
 *
 * cwd is quoted to survive spaces/special chars; a missing cwd falls back to `cd ~`.
 * Returns null for observer sessions so callers suppress the action.
 */
export function buildResumeCommand(session: ResumeSession): string | null {
  const agent = session.agent_type || session.agent
  // Observer wire session ids have no remotely controllable CLI resume path.
  if (isReadOnlyObserverAgent(agent || '')) {
    return null
  }
